...
/* ------------------------ 常量与参数 ------------------------ */
#define BRIDGE_SLAVE_ADDR   0x01u
...
static void bridge_update_extended_feedback_regs(void)
{
    ...
    s_dbg_goal_status &= (uint16_t)~((1u << 10) | (1u << 11));
...
void DM_Bridge_Init(UART_HandleTypeDef *huartx, FDCAN_HandleTypeDef *hfdcan1, uint16_t motor_id){
    ...
    s_regs[REG_FW_VER]   = (1u<<8) | 14u;  /* v1.14: rebuild host/motor bias from AS on boot */
