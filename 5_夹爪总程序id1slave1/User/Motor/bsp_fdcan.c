...
void bsp_can_init(void)
{
	...
	can_filter_init();
	HAL_FDCAN_Start(&hfdcan1);                               // 开启 FDCAN
...
//	HAL_FDCAN_ActivateNotification(&hfdcan1, FDCAN_IT_RX_BUFFER_NEW_MESSAGE, 0);
	HAL_FDCAN_ActivateNotification(&hfdcan1,
                                       0 | FDCAN_IT_RX_FIFO0_NEW_MESSAGE
...
void can_filter_init(void)
{
	...
	fdcan_filter.FilterConfig = FDCAN_FILTER_TO_RXFIFO0;           // 过滤器 0 关联到 FIFO0
	fdcan_filter.FilterID1 = 0x00;                               
	fdcan_filter.FilterID2 = 0x00;

	HAL_FDCAN_ConfigFilter(&hfdcan1,&fdcan_filter);                 // 接收 ID2
	// 拒绝接收未匹配的标准/扩展 ID，不接受远程帧
...
//			case CAN_BR_500K: 	nom_brp=1 ; nom_seg1=139; nom_seg2=20; nom_sjw=20; break; // sample point 87.5%
			case CAN_BR_1M:		nom_brp=4 ; nom_seg1=14 ; nom_seg2=5; nom_sjw=5; break; // sample point 75%
		}
//		dat_brp=4 ; dat_seg1=14; dat_seg2=5; dat_sjw=5;	// 仲裁域默认1M
...
//		pTxHeader.DataLength = FDCAN_DLC_BYTES_64;
	pTxHeader.DataLength = fdcan_len_to_dlc(len);    // 字节长度最多 8
...
{	
	...
	if(HAL_FDCAN_GetRxMessage(hfdcan,FDCAN_RX_FIFO0, &pRxHeader, buf)==HAL_OK)
	{
		*rec_id = pRxHeader.Identifier;
		return fdcan_dlc_to_len(pRxHeader.DataLength); // 接收数据
	}
	return 0;	
}
