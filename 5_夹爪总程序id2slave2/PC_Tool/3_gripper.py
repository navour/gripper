...
class App:
    def __init__(self, root: tk.Tk):
        ...
        self.est_grip_th_var = tk.StringVar(value="0.18")
    ...
    def _build_ui(self):
        ...
        ttk.Button(quick, text="临时解除AS范围(0x0048)", command=self.cmd_bypass_as_range).grid(row=2, column=4, padx=4, pady=4, sticky="ew")
    ...
    def _update_as_motor_feedback_from_regs(self, regs: list[int]):
        ...
        as_bypass = bool(goal_status & (1 << 12))
        ...
        self.debug_var.set(
            ...
            f"first_soft={1 if first_soft else 0}  as_bypass={1 if as_bypass else 0}"
        )
    ...
    def _update_feedback_from_regs(self, regs: list[int]):
        ...
        goal_parts = [
            ...
            f"as_bypass={1 if as_bypass else 0}",
        ]
        try:
            open_deg = float(self.open_deg_var.get())
            close_deg = float(self.close_deg_var.get())
    ...
    def cmd_bypass_as_range(self):
        ok = messagebox.askyesno(
            "临时解除AS范围",
            "仅用于标定找闭合点：将暂时忽略 AS_RANGE_FAULT 约2分钟，保存零点后会自动恢复。\n"
            "请使用低速小角度移动，确认继续？",
        )
        if not ok:
            return
        self._send_and_recv(mb_write_single(self._slave(), REG_AS_RANGE_BYPASS, AS_RANGE_BYPASS_VALUE))
        self.log("已发送：临时解除 AS 范围保护。请低速小角度移动，到闭合点后保存零点。")
